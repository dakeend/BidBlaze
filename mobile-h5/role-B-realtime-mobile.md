...
5. docs/tasks/frontend-agent-tasks.md（M1–M8）+ docs/tasks/backend-agent-tasks.md Task E/F/K
   # Task G 是 Role A 的 outbox publisher；如需对接只读相关约束，不实现 G
...
5. MSW handlers（src/mocks/handlers.ts）：
   ...
   - GET /api/users/me、GET /api/auctions、GET /api/auctions/:id、GET /api/auctions/:id/status
   - fixture 按 openapi.yaml schema + contract-v2.md 示例手写最小样例，字段必须 snake_case
...
5. 业务事件 seq 以 event_outbox.event_seq 为准；WS 网关只转发，不自行生成业务 seq
6. last_seq 只用于 outbox 可补偿事件；viewer_count 单独按 event_id 去重，不推进补偿游标
7. WS token 通过 query 传递仅用于 MVP，代码中必须标 TODO(prod): replace with JWT or safer WS auth
```
...
- 房间级广播；若合同允许匿名连接则只读，否则无 token 拒绝连接
- 多实例部署下，房间路由通过 Redis pub/sub 同步
- 业务事件 seq 以 event_outbox.event_seq 为准，WS 网关不生成 BidAccepted/AuctionStarted 等业务事件
- viewer_count 是软事件，允许不进入 outbox；客户端单独按 event_id 去重，不得因 viewer_count 断档触发 /events 补偿
...
注意：
- viewer_count 事件不应该出现在 /events 响应里（软事件不补偿）。
- 客户端发现业务事件 seq > last_seq+1 时才拉 /events；viewer_count 不推进 last_seq，也不触发补偿。
```
...
3. useServerTime hook（用 /api/auctions/:id/status 的 server_time 校准 Date.now()）
...
说明：这里的 /status 指 GET /api/auctions/:id/status。
...
- 返回 url + width + height + size；url 必须是前端可直接展示的公开访问路径，不能返回本地绝对路径
...
注意 url 形态要和未来切 CDN 兼容，前端别 hardcode 域名，也不要依赖 D:\... 或 /home/... 这类本机路径。
```
...
- 生产环境长期使用 query token；MVP 可按合同从 localStorage 取 token 塞 query，但代码必须标 TODO(prod): replace with JWT or safer WS auth
